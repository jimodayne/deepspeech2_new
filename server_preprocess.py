...
from decoder import batch_decode, batch_label_to_text, \
    list_char_to_string, compute_cer, compute_wer
...
def featurize(
    ...
    return spectrogram_from_file(audio_clip, mode=ModelMode.TEST,
                                 step=step, window=window,
                                 max_freq=max_freq)


def detect_leading_silence(sound, silence_threshold=-5.0,
                           chunk_size=10):
    trim_ms = 0  # ms
    ...
    for i in range(len(text_list)):
        if text_list[i] not in alter_list:
            ...
            if i > 0 and i < len(text_list) - 1:
                if text_list[i - 1] in alter_list and text_list[i + 1] \
                    in alter_list:
                    list_candidate_word = \
                        word_vectors.most_similar([text_list[i - 1],
                            text_list[i + 1]], topn=400)
                    check = 1
            elif i < len(text_list) - 1 and text_list[i + 1] \
                in alter_list:
                list_candidate_word = \
                    word_vectors.most_similar(text_list[i + 1],
                        topn=400)
            ...
            if check == 1:
                for word in list_candidate_word:
                    if editdistance.eval(word[0], text_list[i]) \
                        < min_distance:
                        min_distance = editdistance.eval(word[0],
                                text_list[i])
                        alter_word = word[0]
                text_list[i] = alter_word
            else:
                for word in alter_list:
                    if editdistance.eval(word, text_list[i]) \
                        < min_distance:
                        min_distance = editdistance.eval(word,
                                text_list[i])
                        alter_word = word
                text_list[i] = alter_word
...
def getVoiceToText():
    ...
    with tf.Session() as sess:
        ...
        try:
            saver.restore(sess,
                          tf.train.latest_checkpoint(check_point_directory))
            print ' '
            print 'restore check point success'
            print '-----------------/////////------------------'
        except:
            print ' '
            print ('can not find check point at ',
        ...
        (l, s) = sess.run(deep_speech_model,
                          feed_dict={inputs: audio_input,
                          input_lengths: audio_input_length})
