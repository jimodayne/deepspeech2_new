...
class DataGenerator(object):
    def __init__(self, step=10, window=20, max_freq=22050, desc_file=None):
        """
        Params:
    ...
    def featurize(self, audio_clip, mode):
        """ For a given audio clip, calculate the log of its Fourier Transform
        Params:
            audio_clip(str): Path to the audio clip
        """
        return spectrogram_from_file(
            audio_clip, mode, step=self.step, window=self.window,
    ...
    def iterate(self, audio_paths, texts, minibatch_size, mode,
                max_iters=None):
        ...
        start = 0 #minibatch_size
        for _ in range(k_iters):
            # wait([future])
            # minibatch = future.result()
            # While the current minibatch is being consumed, prepare the next
            minibatch = self.prepare_minibatch(audio_paths[start: start + minibatch_size], texts[start: start + minibatch_size], mode)
            yield minibatch
...
# print(np.shape(datagen.train_texts), np.shape(datagen.train_audio_paths), np.shape(datagen.train_durations))
# abc = DataGenerator.sort_by_duration(datagen.train_durations, datagen.train_audio_paths, datagen.train_texts)
# dataset = datagen.iterate_train()
# print(dataset)
# for i, batch in enumerate(dataset):
#     print("x shape", np.shape(batch['x']))
#     print("y shape", np.shape(batch['y']))
#     print("y", (batch['y'][0]))
#     print("y", np.shape(batch['y'][1]))
#     print("y", (batch['y'][1]))
#     print(np.shape(batch['y']))
#     # print("input_lengths", batch['input_lengths'])
#     print("label_lengths", batch['label_lengths'])
#     print(max(batch['label_lengths']))
#     plt.imshow(batch['x'][10])
#     plt.show()
#     break
