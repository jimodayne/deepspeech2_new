...
from augmentation import augmentation_clean_data
...
def spectrogram_from_file(filename, mode, step=10, window=20, max_freq=None,
                          eps=1e-14):
    ...
    audio, sample_rate = librosa.load(filename, 44100)
    
    #if mode == ModelMode.TRAIN :
    #    audio = augmentation_clean_data(audio)
    ...
    return np.abs(np.transpose(np.log(pxx[:ind, :] + eps)))
